...
class RangeIronCondor(BaseBot):
    ...
    def open_criteria(self, snap: MarketSnapshot) -> Optional[TradeSignal]:
        # Re-enabled once defined-risk support landed: the sizer now keys on the
        # condor's true max loss (wing width − credit), the entry gate accepts a
        # net credit and requires a real one (MIN_CREDIT_PER_SHARE), and the P&L
        # math is sign-general — so a slippage-eroded ~$0 net is skipped rather
        # than opened mis-sized. Watch it in tradeworkz-review; the auto-disable
        # circuit breaker still governs a sustained losing streak.
