...
_QUERY = """
    SELECT symbol, date, open_spot, actual_low, actual_high,
           implied_move, expected_vol_state, vol_state_correct
    FROM daily_forecast
...
def analyze(conn, symbol: Optional[str]) -> dict[str, SymbolStats]:
    ...
    for (sym, _day, open_spot, actual_low, actual_high,
         implied_move, expected_vol_state, committed_correct) in db_rows:
    ...
    for sym, rows in per_symbol.items():
        ...
        for r in rows:
            ...
            _, cal_b, cal_ok = _regrade_ratio(r, RANGE_OVER_SIGMA * basis)
            ...
            # A row scores only when a morning call was committed to grade.
...
def _format_report(stats: dict[str, SymbolStats]) -> str:
    lines = [
        "=" * 78,
        "VOL-SCALE REGRADE BACKTEST — old vs corrected range/σ scale (READ-ONLY)",
        "=" * 78,
        "",
        "Holds each committed morning vol call fixed and re-grades the realized",
        "bucket under three denominators. OLD = ÷implied (the bug); FIXED =",
        "÷(√(8/π)·implied); CALIB = FIXED with the per-symbol basis the nightly",
        "cron converges to. The committed forecasts are immutable and UNCHANGED.",
        "",
    ]
    if not stats:
    ...
    for sym in sorted(stats):
        ...
        lines.append(
            f"   vol_correct_rate  OLD ÷implied     : {_fmt_rate(s.old_rate)} "
            f"(n={s.n_gradeable})"
        )
        lines.append(
            f"   vol_correct_rate  FIXED ÷1.6·impl  : {_fmt_rate(s.fixed_rate)}"
        )
        lines.append(
            f"   vol_correct_rate  CALIB (basis {s.calib_basis}): {_fmt_rate(s.calib_rate)}"
        )
        lines.append(
            f"   committed (as stored)             : {_fmt_rate(s.committed_rate)} "
            f"(n={s.committed_scored})"
        )
        lines.append(
            f"   realized buckets OLD  : {dict(sorted(s.old_buckets.items()))}"
        )
        lines.append(
            f"   realized buckets CALIB: {dict(sorted(s.calib_buckets.items()))}"
        )
        lines.append("")
    return "\n".join(lines)
