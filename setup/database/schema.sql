...
-- SAME contract: vendor SESSION-CUMULATIVE, resets to 0 at the cash
-- open, monotonic intraday.  Period (per-bucket) values for any of
-- them are derived via ``LAG()`` deltas inside the same ET session —
-- the canonical materialization lives in ``flow_contract_facts``.
-- Per-row invariant: ``ask_volume + mid_volume + bid_volume == volume``
-- modulo the opening-auction carve-out (09:30 ET bucket routes all
-- volume to ``mid_volume`` because the auction cross price is not
-- comparable to NBBO).  The ingestion writer maintains these as
-- session-cumulative running totals in a per-contract in-memory
-- accumulator and the upsert uses ``GREATEST`` on every monotonic
-- column for idempotent replace semantics.
...
COMMENT ON COLUMN option_chains.ask_volume IS
    'Session-cumulative Lee-Ready ask-side (buyer-initiated) classified volume. Resets at cash open, monotonic intraday. Per-bucket flow = LAG delta within the ET session.';
COMMENT ON COLUMN option_chains.mid_volume IS
    'Session-cumulative mid/indeterminate classified volume. Includes the opening-auction bucket in full. Resets at cash open, monotonic intraday.';
COMMENT ON COLUMN option_chains.bid_volume IS
    'Session-cumulative Lee-Ready bid-side (seller-initiated) classified volume. Resets at cash open, monotonic intraday. Per-bucket flow = LAG delta within the ET session.';
